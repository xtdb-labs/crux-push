...
(comment
  (let [schema (jinx/schema (edn/read-string (slurp (io/resource "rlws/iot_cloud/schema.edn"))))
        input (->
               (edn/read-string (slurp "../docs/test-alert.edn"))
               (validate-message schema)
               (cruxify-message
                (get-in (jinxresolv/expand-document schema nil) ["anyOf" 1])))]
    (flatten-crux-documents input)))

(comment
  (jinxresolv/expand-document (jinx/schema (edn/read-string (slurp (io/resource "rlws/iot_cloud/schema.edn")))) nil))

(comment
  (edn/read-string (slurp "../docs/test-alert.edn" #_"../docs/logical-gateway.edn")))
...
(comment
  (let [schema (jinx/schema (edn/read-string (slurp (io/resource "rlws/iot_cloud/schema.edn"))))
    ...
    (crux/submit-tx
     (dev/crux)
     (vec (for [doc (map keywordize-keys docs)]
            [:crux.tx/put doc])))))

(comment
  (crux/q
   (crux/db (dev/crux))
   {:find ['e] :where [['e :crux.db/id 'o]] :full-results? true}))

(comment
  (crux/q
   (crux/db (dev/crux))
   {:find ['e 'dealer]
    :where [['e :name "Fred's Peanuts"]
            ['e :servicedByDealership 'd]
            ['d :name 'dealer]]
    :full-results? true}))
